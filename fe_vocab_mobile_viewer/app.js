...
async function handleFile(event) {
  ...
  statusEl.textContent = `${rows.length}語を読み込みました`;
}
...
function applyFilters() {
  ...
  filtered = rows.filter(row => {
    ...
    const matchField = !field || row.field === field;
  ...
  termCard.innerHTML = `
    ...
    ${relatedItems.length ? `
      <section class="section">
        <h2>関連語</h2>
        <div class="related">
          ${relatedItems.map(v => `<span>${escapeHTML(v)}</span>`).join("")}
        </div>
      </section>
    ` : ""}
  `;
...
function renderList() {
  ...
  filtered.forEach((row, index) => {
    ...
    button.innerHTML = `
      <div class="list-title">${escapeHTML(row.term)}</div>
      <div class="list-sub">${escapeHTML(row.short || row.detail || row.field || "")}</div>
    `;
